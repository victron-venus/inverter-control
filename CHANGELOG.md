...
### Fixed
- Keep background telemetry polling running when an optional D-Bus service
  disappears and its CLI fallback returns no result. Preserve the existing
  zero fallback instead of raising an exception before later reads finish.

### Validation
- Cover unavailable and empty CLI replies, service backoff, continued polling
  of another PV service and battery energy, and recovery on a later poll.
