...
class DvccCalculator:
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        Args:
            config: Dictionary with all parameters from main config
        """
        cfg = config or {}

        self.cell_count = cfg.get("DVCC_CELL_COUNT", 16)
        self._max_charge_current = cfg.get("DVCC_MAX_CHARGE_CURRENT", 100.0)
        self._max_discharge_current = cfg.get("DVCC_MAX_DISCHARGE_CURRENT", 120.0)
        self.cell_max_voltage = cfg.get("DVCC_CELL_MAX_VOLTAGE", 3.65)

        # Cell voltage thresholds
        self.cell_full_current = cfg.get("DVCC_CELL_FULL_CURRENT", 3.40)
        self.cell_start_limit = cfg.get("DVCC_CELL_START_LIMIT", 3.45)
        self.cell_balance_voltage = cfg.get("DVCC_CELL_BALANCE_VOLTAGE", 3.50)
        self.cell_near_full = cfg.get("DVCC_CELL_NEAR_FULL", 3.55)
        self.cell_cutoff = cfg.get("DVCC_CELL_CUTOFF", 3.60)

        # Imbalance thresholds
        self.imbalance_start = cfg.get("DVCC_IMBALANCE_START_LIMIT", 0.05)
        self.imbalance_aggressive = cfg.get("DVCC_IMBALANCE_AGGRESSIVE", 0.10)
        self.imbalance_critical = cfg.get("DVCC_IMBALANCE_CRITICAL", 0.20)

        # Temperature thresholds (°C)
        self.temp_charge_min = cfg.get("DVCC_TEMP_STOP_CHARGE", 0.0)
        self.temp_charge_reduced = cfg.get("DVCC_TEMP_REDUCED", 5.0)
        self.temp_charge_optimal = cfg.get("DVCC_TEMP_FULL_CURRENT_MIN", 10.0)
        self.temp_charge_limit = cfg.get("DVCC_TEMP_FULL_CURRENT_MAX", 40.0)
        self.temp_charge_stop = cfg.get("DVCC_TEMP_STOP_CHARGE_HIGH", 50.0)
        self.temp_discharge_min = cfg.get("DVCC_TEMP_DISCHARGE_MIN", -20.0)
        self.temp_discharge_reduced = cfg.get("DVCC_TEMP_DISCHARGE_REDUCED", -10.0)

        # SoC thresholds
        self.soc_reduce_start = cfg.get("DVCC_SOC_REDUCE_START", 95.0)
        self.soc_reduce_factor = cfg.get("DVCC_SOC_REDUCE_FACTOR", 0.5)
        self.soc_discharge_stop = cfg.get("DVCC_SOC_DISCHARGE_STOP", 5.0)
        self.soc_discharge_reduced = cfg.get("DVCC_SOC_DISCHARGE_REDUCED", 15.0)

        self.min_charge_current = cfg.get("DVCC_MIN_CHARGE_CURRENT", 2.0)

        # Rate limiting
        self.ccl_change_rate = cfg.get("DVCC_CCL_CHANGE_RATE", 10.0)
        self.dcl_change_rate = cfg.get("DVCC_DCL_CHANGE_RATE", 15.0)
    ...
    def calculate_ccl_from_cell_voltage(self, max_cell: float | None) -> tuple[float, str]:
        ...
        min_cc = self.min_charge_current

        # Cell cutoff - stop charging
        if v >= self.cell_cutoff:
            ccl, reason = 0.0, f"cell_overvoltage_{v:.3f}V"
        # Near full - minimal current for balancing
        elif v >= self.cell_near_full:
            factor = 1.0 - (v - self.cell_near_full) / (self.cell_cutoff - self.cell_near_full)
            ccl, reason = max(0.0, min_cc * factor), f"tail_charge_{v:.3f}V"
        # Balance voltage - aggressive reduction
        elif v >= self.cell_balance_voltage:
            factor = 1.0 - (v - self.cell_balance_voltage) / (
                self.cell_near_full - self.cell_balance_voltage
            )
    ...
    def calculate_ccl_from_imbalance(self, cell_delta: float | None) -> tuple[float, str]:
        ...
        if cell_delta <= self.imbalance_start:
            ccl, reason = max_cc, "balanced"
        elif cell_delta >= self.imbalance_critical:
            ccl, reason = min_cc, f"critical_imbalance_{cell_delta:.3f}V"
        elif cell_delta >= self.imbalance_aggressive:
            factor = 1.0 - (cell_delta - self.imbalance_aggressive) / (
                self.imbalance_critical - self.imbalance_aggressive
            )
            ccl, reason = min_cc + (max_cc * 0.30 - min_cc) * factor, f"imbalance_{cell_delta:.3f}V"
        else:
            # Start limiting zone
            factor = 1.0 - (cell_delta - self.imbalance_start) / (
                self.imbalance_aggressive - self.imbalance_start
            )
    ...
    def calculate_ccl_from_temperature(
        ...
        if temp <= self.temp_charge_min:
            ccl, reason = 0.0, f"too_cold_{temp:.1f}C"
        elif temp >= self.temp_charge_stop:
        ...
        # Cool - reduce to ~50%
        elif temp < self.temp_charge_optimal:
            factor = (temp - self.temp_charge_reduced) / (
                self.temp_charge_optimal - self.temp_charge_reduced
            )
            factor = max(0.0, min(1.0, factor))
            ccl, reason = (
                max(self._max_charge_current * (0.1 + 0.4 * factor), self.min_charge_current),
                f"cool_{temp:.1f}C",
            )
        # Hot - reduce linearly
    ...
    def calculate_ccl_from_soc(self, soc: float | None) -> tuple[float, str]:
        ...
        if soc is None or soc < self.soc_reduce_start:
            return self._max_charge_current, "soc_ok"

        if soc >= 100.0:
            return self._max_charge_current * self.soc_reduce_factor, "soc_100"

        # Linear reduction from 100% to REDUCE_FACTOR
        factor = 1.0 - (soc - self.soc_reduce_start) / (100.0 - self.soc_reduce_start) * (
            1.0 - self.soc_reduce_factor
        )
    ...
    def calculate_dcl_from_temperature(
        ...
        if temp <= self.temp_discharge_min:
            return 0.0, f"too_cold_discharge_{temp:.1f}C"

        if temp < self.temp_discharge_reduced:
            factor = (temp - self.temp_discharge_min) / (
                self.temp_discharge_reduced - self.temp_discharge_min
            )
            factor = max(0.0, min(1.0, factor))
            return max(
    ...
    def calculate_dcl_from_soc(self, soc: float | None) -> tuple[float, str]:
        ...
        if soc is None or soc > self.soc_discharge_reduced:
        ...
        factor = (soc - self.soc_discharge_stop) / (
            self.soc_discharge_reduced - self.soc_discharge_stop
        )
        factor = max(0.0, min(1.0, factor))
    ...
    def _rate_limit(
        ...
        max_ccl_change = self.ccl_change_rate * dt
        max_dcl_change = self.dcl_change_rate * dt
    ...
    def calculate(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        Returns:
            Dict with ccl, dcl, cvl, and diagnostic reasons
        """
