...
class DvccCalculator:
    ...
    def calculate(self, data: dict[str, Any]) -> DvccLimits:
        ...
        ccl_values: list[tuple[float, str]] = []

        ccl_voltage, reason = self.calculate_ccl_from_cell_voltage(max_cell)
        ccl_values.append((ccl_voltage, reason))

        ccl_imbalance, reason = self.calculate_ccl_from_imbalance(cell_delta)
        ccl_values.append((ccl_imbalance, reason))

        ccl_temp, reason = self.calculate_ccl_from_temperature(min_temp, max_temp)
        ccl_values.append((ccl_temp, reason))

        ccl_soc, reason = self.calculate_ccl_from_soc(soc)
        ccl_values.append((ccl_soc, reason))

        # Take minimum CCL (most restrictive)
        ccl, ccl_reason = min(ccl_values, key=lambda x: x[0])

        # Calculate DCL from all sources - take minimum
        dcl_values: list[tuple[float, str]] = []

        dcl_voltage, reason = self.calculate_dcl_from_cell_voltage(min_cell)
        dcl_values.append((dcl_voltage, reason))

        dcl_temp, reason = self.calculate_dcl_from_temperature(min_temp, max_temp)
        dcl_values.append((dcl_temp, reason))

        dcl_soc, reason = self.calculate_dcl_from_soc(soc)
        dcl_values.append((dcl_soc, reason))

        dcl, dcl_reason = min(dcl_values, key=lambda x: x[0])
